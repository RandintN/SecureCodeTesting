...
# Enroll CA Admin
docker exec ca.$COMPANY_DOMAIN fabric-ca-client enroll -d -u https://$CA_ADMIN_USER:$CA_ADMIN_PASSWORD@$CA_ADDRESS_PORT --csr.hosts ca.$COMPANY_DOMAIN
...
sleep 3
# peer channel fetch 0 n2medchannel.block -o 192.168.65.89:7050 -c n2medchannel --tls --cafile $CORE_PEER_TLS_ROOTCERT_FILE
######################################
#               PEER                 #
######################################
...
docker exec ca.$COMPANY_DOMAIN fabric-ca-client enroll -d -u https://peer0.$COMPANY_DOMAIN:peerpw@$CA_ADDRESS_PORT --csr.hosts peer0.$COMPANY_DOMAIN --myhost peer0.$COMPANY_DOMAIN -M $NODE_DIRECTORY/$PEER_DIRECTORY/peers/peer0.$COMPANY_DOMAIN/msp

# Enroll peer identity to get TLS certificates
docker exec ca.$COMPANY_DOMAIN fabric-ca-client enroll -d -u https://peer0.$COMPANY_DOMAIN:peerpw@$CA_ADDRESS_PORT --csr.hosts peer0.$COMPANY_DOMAIN,$IP_ADDRESS --myhost peer0.$COMPANY_DOMAIN --enrollment.profile tls -M $NODE_DIRECTORY/$PEER_DIRECTORY/peers/peer0.$COMPANY_DOMAIN/tls

# Enroll Admin identities for the Peer MSP
docker exec ca.$COMPANY_DOMAIN fabric-ca-client enroll -d -u https://Admin@peer0.$COMPANY_DOMAIN:$PEER_PASSWORD@$CA_ADDRESS_PORT --myhost peer0.$COMPANY_DOMAIN -M $NODE_DIRECTORY/$PEER_DIRECTORY/users/Admin@peer0.$COMPANY_DOMAIN/msp
...
docker exec ca.$COMPANY_DOMAIN fabric-ca-client enroll -d -u https://User@peer0.$COMPANY_DOMAIN:userpw@$CA_ADDRESS_PORT --myhost peer0.$COMPANY_DOMAIN -M $NODE_DIRECTORY/$PEER_DIRECTORY/users/User@peer0.$COMPANY_DOMAIN/msp
...
docker exec ca.$COMPANY_DOMAIN cp -r $NODE_DIRECTORY/$ORDERER_MSP/users/Admin@orderer.$COMPANY_DOMAIN/msp/admincerts/ $NODE_DIRECTORY/$ORDERER_MSP/orderers/orderer.$COMPANY_DOMAIN/msp/admincerts
